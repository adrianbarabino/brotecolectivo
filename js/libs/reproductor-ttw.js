 ...
 *
 * Modificaciones por Adrian Barabino para Brote Colectivo
 * 20/07/2012 a las 1:40 AM
 * Se creó el elemento 'Genero' y se añadió el nombre del artista a la lista de temas.
 *
 * 27/11/2012 a las 15:48 PM
 * Se añade la visualización de letras en los temas, solo visible en la lista de temas. 
 * 
 */
...
(function($) {
    $.fn.ttwMusicPlayer = function(playlist, userOptions) {
        ...
        cssSelector = {
            ...
            artist:'.artist',
            genero:'.genero',
            generoOuter:'.genero-outer',
            letra:'.letra',
            letraOuter:'.letra-outer',
        ...
        playlistMgr = function() {
            ...
            function buildPlaylist() {
                ...
                for (var j = 0; j < myPlaylist.length; j++) {
                    ...
                    $track.find(cssSelector.genero).html(trackGenero(j));
                    $track.find(cssSelector.letra).html(trackLetra(j));
        ...
        interfaceMgr = function() {
            ...
            var $player, $title, $artist, $genero, $letra, $albumCover;
            ...
            function init() {
                $player = $(cssSelector.player),
                        ...
                        $genero = $player.find(cssSelector.genero),
                        $letra = $player.find(cssSelector.letra),
            ...
            function buildInterface() {
                ...
                markup = '<div class="ttw-music-player">' +
                        ...
                        '            <p class="genero-outer">Género:  <span class="genero"></span></p>' +
                        '            <div class="rating">' +
                        ...
                        '            </div>' +
                        '        </div>' +
                        '        <div title="Lista de Reproducción" class="boton-playlist reproductor-tooltip"></div><div class="player-controls">' +
                        '            <div class="main">' +
                        '                <div title="Tema anterior" class="previous jp-previous reproductor-tooltip"></div>' +
                        '                <div title="Reproducir" class="play jp-play reproductor-tooltip"></div>' +
                        '                <div title="Pausa" class="pause jp-pause reproductor-tooltip"></div>' +
                        '                <div title="Siguiente tema" class="next jp-next reproductor-tooltip"></div>' +
                        ...
                        '                    <span title="Barra de volumen" class="jp-volume-bar reproductor-tooltip"></span>' +
                        '                    <div class="volume-container"><span class="jp-volume-bar-value"></span></div>' +
                        ...
                        '    <div id="container-tracklist"><div class="tracklist">' +
                        '        <ol class="tracks"> </ol>' +
                        '    </div></div>' +
            ...
            function setGenero() {
                ...
                generoactual = myPlaylist[current].genero;
                var idactual;
                idactual = myPlaylist[current].oga;

                idverdadero2 = idactual.substring(11,15);
                idverdadero1 = idverdadero2.split('-');

                 idverdadero = idverdadero1[0];


                if (isUndefined(myPlaylist[current].genero))
                    $genero.parent(cssSelector.generoOuter).animate({opacity:0}, 'fast');
                else {
                    $genero.html(myPlaylist[current].genero).parent(cssSelector.generoOuter).animate({opacity:1}, 'fast');
                }
            }
            ...
            function setLetra() {
                ...
                if (isUndefined(myPlaylist[current].letra))
                    $letra.parent(cssSelector.letraOuter).animate({opacity:0}, 'fast');
                else {
                    $letra.html(myPlaylist[current].letra).parent(cssSelector.letraOuter).animate({opacity:1}, 'fast');
                }
            }
        ...
        function trackLetra(index) {
            if (!isUndefined(myPlaylist[index].letra))
                return myPlaylist[index].letra;
            else if (!isUndefined(myPlaylist[index].mp3))
                return fileName(myPlaylist[index].mp3);
            else if (!isUndefined(myPlaylist[index].oga))
                return fileName(myPlaylist[index].oga);
